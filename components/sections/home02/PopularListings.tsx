...
function PopularListings() {
  const [activeTab, setActiveTab] = useState(home02PopularListingTabs[0]);

  const filteredCars = useMemo(() => {
    return home02PopularListings.filter((car) =>
      car.bodyType?.includes(activeTab),
    );
  }, [activeTab]);
        ...
        <div className="container">
          ...
          <div className="flat-tabs themesflat-tabs wow fadeInUp">
            <div className="box-tab center flex-five">
              <ul className="menu-tab tab-title style1 flex magin-auto  mb-40">
                {home02PopularListingTabs.map((tab) => (
                  <li
                    key={tab}
