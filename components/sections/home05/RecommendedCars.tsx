...
import { home04BrandTabs, home05RecommendedCars, getCarDetailHref } from "@/data/cars";
import ListingCardActions from "@/components/common/ListingCardActions";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation, Pagination } from "swiper/modules";

function RecommendedCars() {
  const [activeTab, setActiveTab] = useState(home04BrandTabs[0]);

  const filteredCars = useMemo(() => {
    return home05RecommendedCars.filter((car) =>
      car.brandType?.includes(activeTab),
    );
  }, [activeTab]);

  return (
    <>
      <section className="tf-section bg-black">
        <div className="container">
          ...
          <div className="flat-tabs themesflat-tabs wow fadeInUp">
            <div className="box-tab center flex-five">
              <ul className="menu-tab tab-title style3 flex magin-auto">
                {home04BrandTabs.map((tab) => (
                  <li
                    key={tab}
                ...
                >
                  {filteredCars.map((car) => (
                    <SwiperSlide className="swiper-slide" key={car.id}>
                      <div className="box-car-list hv-one style-4">
                  ...
                  <span className="d-flex sw-dot-default swiper-pagination-black justify-content-center" />
                </Swiper>
              </div>
            </div>
          </div>
