...
function NearbyUsedCars() {
  const [activeTab, setActiveTab] = useState(home04NearbyUsedPriceTabs[0]);

  const filteredCars = useMemo(() => {
    return home04NearbyUsedCars.filter((car) =>
      car.priceRange?.includes(activeTab),
    );
  }, [activeTab]);
      ...
      <section className="tf-section">
        <div className="container">
          <div className="heading-section flex align-center justify-space flex-wrap gap-20 wow fadeInUp">
            <h2 className="heading-tittle">Get trusted used cars nearby</h2>
            <Link
              href={`/listing-grid`}
          ...
          <div className="flat-tabs themesflat-tabs wow fadeInUp">
            <div className="box-tab center">
              <ul className="menu-tab tab-title style flex">
                {home04NearbyUsedPriceTabs.map((tab) => (
                  <li
                    key={tab}
                ...
                >
                  {filteredCars.map((car) => (
                    <SwiperSlide className="swiper-slide" key={car.id}>
                      <div className="box-car-list hv-one style-2 style-dark">
