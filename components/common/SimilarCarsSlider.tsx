...
type SimilarCarsSliderProps = {
  currentCar: Car;
};

export default function SimilarCarsSlider({ currentCar }: SimilarCarsSliderProps) {
  const { cars: realCars, loading, error } = useSimilarListings(currentCar);
  const isRealListing = Boolean(currentCar.publicId);
  const cars = isRealListing ? realCars : similarCars;

  if (isRealListing && loading) {
    return <p>Loading similar cars...</p>;
  }

  if (isRealListing && error) {
    return <p>{error}</p>;
  }

  if (isRealListing && cars.length === 0) {
    return <p>No similar cars found right now.</p>;
  }
  ...
  return (
    <Swiper
      ...
      slidesPerGroup={1}
      navigation={{
        nextEl: ".similar-cars-nav-next",
        prevEl: ".similar-cars-nav-prev",
      }}
      pagination={{
        el: ".similar-cars-pagination",
        clickable: true,
      }}
      breakpoints={{
        575: { slidesPerView: 1.5, spaceBetween: 12, slidesPerGroup: 1 },
    ...
    >
      ...
      <span className="d-flex d-xl-none sw-dot-default similar-cars-pagination justify-content-center" />
      <div className="nav-next-categories similar-cars-nav-next" />
      <div className="nav-prev-categories similar-cars-nav-prev" />
    </Swiper>
  );
}
